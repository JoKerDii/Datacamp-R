---
title: "Datacamp_Importing_data_in_R(Part1)__Importing_Excel_data"
author: "dizhen"
date: "2019Äê4ÔÂ4ÈÕ"
...
1. Load the readxl package using library(). It's already installed on DataCamp's servers.

2. Use excel_sheets() to print out the names of the sheets in urbanpop.xlsx.
...
1. Use lapply() in combination with excel_sheets() and read_excel() to read all the Excel sheets in "urbanpop.xlsx". Name the resulting list pop_list.
...
2. Import the first Excel sheet of urbanpop_nonames.xlsx; this time, use the cols vector that has already been preparedfor you to specify the column names. Store the resulting data frame in pop_b.
...
1. Load the gdata package with library(). gdata and Perl are already installed on DataCamp's Servers.

2. Import the second sheet, named "1967-1974", of "urbanpop.xls" with read.xls(). Store the resulting data frame as urban_pop.

3. Print the first 11 observations of urban_pop with head().
...
3. Use na.omit() on the urban data frame to remove all rows that contain NA values. Store the cleaned data frame as urban_clean.
